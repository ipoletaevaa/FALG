...
void record_action(UndoManager* manager, ActionType type, Liver* liver_before, Liver* liver_after, size_t index) {
    ...
    if (liver_before) {
        if (copy_liver(liver_before, &action->liver_before) != LIVER_SUCCESS) {
            free(action);
            return;
        }
    } else {
    ...
    if (liver_after) {
        if (copy_liver(liver_after, &action->liver_after) != LIVER_SUCCESS) {
            if (action->liver_before) destroy_liver(action->liver_before);
            free(action);
            return;
        }
    ...
    if (manager->redo_stack) {
        Node* current = manager->redo_stack->head;
        while (current != NULL) {
            Action* redo_action = (Action*)current->data;
            if (redo_action) {
                if (redo_action->liver_before) destroy_liver(redo_action->liver_before);
                if (redo_action->liver_after) destroy_liver(redo_action->liver_after);
                free(redo_action);
            }
            current = current->next;
        }
